...
const _fk = (key, subKey) => {
  const rk = [];
  const lk = [];
  for (let i of key.keys()) { // 왼쪽 4bit, 오른쪽 4bit 분리
...
const _F = (rk, subKey) => {
  ...
  for (let i of list.keys()) { // ep & p 과정
    ep[i] = rk[list[i] - 1];
    p[i] = ep[i] ^ subKey[i];
  }

  const index = [ // p mapping
  ...
  const sbox = _p(_setBit(s0[_convertTen(index[0])][_convertTen(index[1])], 2)
